...
public class RandomizedQueue<Item> implements Iterable<Item> {
    ...
    public void enqueue(Item item) {
        ...
        if (numberOfItems == 0) resize(1); // O(M), where M is numberOfItems
        else if (numberOfItems == itemArray.length) resize(2 * itemArray.length); // O(M), where M is numberOfItems
        itemArray[numberOfItems++] = item; // O(N)
        realNumberOfItems++;
    }
    ...
    public Item dequeue() {
        if (realNumberOfItems > 0) {
            ...
            if (--realNumberOfItems == itemArray.length / 4) resize(itemArray.length / 2); // O(M), where M is numberOfItems
            return randomOutput.randomItem;
        } else {
            throw new NoSuchElementException();
        }
